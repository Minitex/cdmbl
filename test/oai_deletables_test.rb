...
module CDMBL
  describe OaiDeletables do
    ...
    let(:identifiers) { ['collection123:1', 'collection124:2'] }
    ...
    describe '#deletables' do
      it 'makes requests using a persistent connection' do
        OaiClient.stub(:persistent, -> (domain, &block) {
          ...
          oai_record_klass.expect(
            :new,
            oai_record_instance_1,
            [],
            oai_client: oai_client,
            identifier: 'oai:blah:collection123/1'
          )
          oai_record_klass.expect(
            :new,
            oai_record_instance_2,
            [],
            oai_client: oai_client,
            identifier: 'oai:blah:collection124/2'
          )
          ...
          _(deletables).must_equal ['collection124:2']
