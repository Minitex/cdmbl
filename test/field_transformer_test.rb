  ...
  describe FieldTransformer do
    ...
    it 'calls the field formatter for each mapping' do
      formatter.expect(
        ...
        formatter_object,
        [],
        value: '  The Stars My Destination  ',
        formatters: [CDMBL::DefaultFormatter]
      )
      ...
      transformer = FieldTransformer.new(
    ...
    it 'returns empty hash when formatter returns nil' do
      class NullFormatter
        def self.format(doc)
          nil
        end
      ...
      _(transformer.reduce).must_equal({})
    end
  end
end
